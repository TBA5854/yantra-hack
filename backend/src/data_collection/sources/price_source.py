...
class MockPriceSource:
    """
    Mock price source for testing without CoinGecko API.
    ...
    async def fetch_price(
        ...
        # Add tiny random variation (±0.1%)
        price = base_price * (1.0 + random.uniform(-0.001, 0.001))
        
        # Occasional depeg event (1% chance)
        if random.random() < 0.01:
            depeg_dir = 1 if random.random() < 0.5 else -1
            price = base_price + (depeg_dir * random.uniform(0.01, 0.05))
            logger.warning(f"[MOCK] Depeg event simulated for {coin}: ${price:.4f}")
        ...
        event = RiskEvent(
            ...
            price=price,
            volume=random.uniform(10_000_000, 50_000_000),
            block_number=None,
            tx_hash=None
        )
        
        logger.info(f"[MOCK] Fetched price for {coin}: ${price:.4f}")
        return event
    ...
    async def fetch_prices_batch(
        ...
        """Generate mock prices for multiple coins."""
        events = []
        for coin in coins:
            event = await self.fetch_price(coin, chain)
            if event:
                events.append(event)
        return events


# Singleton instantiation with fallback
api_key = config.PRICE_SOURCES["coingecko"].get("api_key")
...
should_use_real_api = (
    ...
    and len(api_key) > 5
    and "your_key" not in api_key.lower()
)

if should_use_real_api:
    price_source = PriceSource()
    logger.info("✅ Using real CoinGecko API for prices")
else:
    price_source = MockPriceSource()
    logger.info(f"⚠️ Using MockPriceSource (API key present: {bool(api_key)})")
